...
$(document).ready(function(){
    ...
    let id = null;
    let module_label = 'Business Permit';
    let module = 'business_permit';
    ...
    $(document).on('click', '.btn-edit', async function() {
        ...
        if(response.success) {
            ...
            showModalDetails($('#edit-modal'), response.data.business_permit);
    ...
    $(document).on('select2:select', '.sanitary_inspector_id', async function() {
        let type = $(this).attr('data-type');
        runLoader($(`#${type}-modal .modal-content`));
        let business_permit_id = $(this).val();

        let payload = {
            module: 'signature',
            action: 'health_official',
            csrf_token: app_csrf_token,
            id: business_permit_id,
        };
        let response = await Api.show(payload);
    ...
    $(document).on('click', '.btn-print', function() {
        let business_permit_id = $(this).attr('data-id');
        window.location.href = `${base_url}/views/business-permits/print.php?id=${business_permit_id}`;
    });
