$(document).ready(function(){
    ...
    async function preload() {
        ...
        runLoader($('body'));
        let sanitary_permit_id = $('meta[name="sanitary_permit_id"]').attr('content');

        let payload = {
            module: 'sanitary_permit',
            action: 'prints',
        ...
        if(data.success) {
            if(data.data.is_passed == 0) {
                window.location.href= base_url + '/views/sanitary-permits/';
            }
